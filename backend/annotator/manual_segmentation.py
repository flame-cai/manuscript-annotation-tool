...
#     img = cv2.imread(img_file, cv2.IMREAD_UNCHANGED)  # Load as is (grayscale or color)
    
#     if img is None:
#         raise ValueError(f"Error loading image: {img_file}")
...
# def assign_labels_to_bounding_boxes(bounding_boxes, points, labels, image):
#     # Initialize labeled bounding boxes
#     labeled_bboxes = []
...
#                 assigned_label = label  # Assign the first found label
#                 break  # Stop checking once a label is assigned

#         labeled_bboxes.append((x_min, y_min, w, h, assigned_label))
...
def assign_labels_and_plot(bounding_boxes, points, labels, image, output_path="output.png"):
    ...
    for bbox in bounding_boxes:
        ...
        if assigned_label is not None:
            cv2.putText(image, str(assigned_label), (x_min, y_min - 5), 
...
lineheight_baseline_percentile=80
...
for det,image,file_name in zip(heatmaps_images,inp_images,inp_file_names):
    ...
    #peaks, _ = find_peaks(ys, height=thres,distance=det.shape[0]/100,width=5)
    #bounding_boxes = gen_bounding_boxes(det,peaks, lineheight_baseline_percentile, binarize_threshold)
    #Assigning labels to bounding boxes
